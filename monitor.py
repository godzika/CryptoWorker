...
def get_transaction_receipt_with_retries(eth: EthereumService, tx_hash: str, max_attempts=RETRY_ATTEMPTS, delay=RETRY_DELAY):
    ...
    while attempts < max_attempts:
        try:
            ...
            logging.info(f"Receipt for tx {tx_hash} not found, attempt {attempts+1}/{max_attempts}")
            time.sleep(delay)
            attempts += 1
            continue
        except Exception as e:
            logging.error(f"Unexpected error fetching tx {tx_hash}: {e}")
            break
    return None
...
def monitor_pending_transactions(db, eth):
    ...
    logging.info("Monitor started. Checking for pending transactions...")

    try:
        while True:
            pending_txs = db.fetch_pending_transactions()
            if not pending_txs:
                logging.info("No pending transactions found. Sleeping 10 seconds...")
                time.sleep(10)
                continue
            ...
            for tx in pending_txs:
                ...
                if not tx_hash or not isinstance(tx_hash, str) or not tx_hash.startswith('0x'):
                    logging.warning(f"Invalid tx_hash in DB for tx_id={tx.internal_id}: {tx_hash}")
                ...
                logging.info(f"Checking transaction {tx_hash} (tx_id={tx.internal_id})")
                receipt = get_transaction_receipt_with_retries(eth, tx_hash)

                if receipt is None:
                    logging.warning(f"Receipt not found for tx_id={tx.internal_id} after {RETRY_ATTEMPTS} attempts. Marking as STALE_PENDING.")
                    db.update_transaction_status(
                        tx.internal_id,
        ...
        logging.critical(f"Monitor crashed: {ex}", exc_info=True)
    finally:
        db.close()
